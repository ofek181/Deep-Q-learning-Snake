...
class DQNAgent:
    ...
    def _take_scores(self, i: int, num_steps: int, e: float, max_step: int):
        global start_time
        global times
        times = 1000
    ...
    def train(self, num_steps: int = 200000, batch_size: int = 100,
              learning_rate: float = 0.0005, y: float = 0.9,
              start_e: float = 1.0, end_e: float = 0.01,
              start_max_step: int = 50, exploration_steps: int = 100000, save_name: str = None):
        ...
        for i in range(num_steps):
            ...
            self._take_scores(i, num_steps, e, max_step)
...
def main():
    agent = DQNAgent(fps=2000, max_step=2000)
    agent.train(save_name="15x10x200000x2000")
    # agent.load_model("15x10x200000x2000")
    # agent.test()
